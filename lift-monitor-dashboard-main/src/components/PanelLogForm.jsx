...
const PanelLogForm = ({ initialData = null, building, onSubmit, onCancel }) => {
  ...
  }, [initialData, user, building, getAccessibleBuildings]);
  ...
  const validateForm = () => {
    ...
    if (formData.panelType === "HT" || formData.panelType === "BOTH") {
      ...
      const trCount = config.hasTr3 ? 3 : 2;
      for (let i = 1; i <= trCount; i++) {
        ...
        if (config.hasHtOilTemp && !ht[tr].oilTemp) return `Outgoing Tr-${i} Oil Temp is required`;
        if (config.hasHtTap && !ht[tr].tap) return `Outgoing Tr-${i} Tap is required`;
      }
    }
    ...
    if (formData.panelType === "LT" || formData.panelType === "BOTH") {
      ...
      const incomerCount = config.hasInc3 ? 3 : 2;
      for (let i = 1; i <= incomerCount; i++) {
        const incomer = `incomer${i}`;
        ...
        if (config.hasLtTap && !inc.tap) return `${incomer} Tap is required`;
      ...
      <form onSubmit={handleSubmit} className="panel-log-form">
        ...
        {(formData.panelType === "LT" || formData.panelType === "BOTH") && (
          <div className="form-section">
            ...
            {["incomer1", "incomer2", "incomer3"]
              .filter((incomer, index) => index < 2 || config.hasInc3)
              .map((incomer, index) => (
