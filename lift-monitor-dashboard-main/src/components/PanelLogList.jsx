...
const PanelLogList = ({ onEdit, onCreateNew }) => {
  ...
  // Helper function to safely parse powerFailure data
  const parsePowerFailure = (powerFailure) => {
    if (!powerFailure) return [];
    if (Array.isArray(powerFailure)) return powerFailure;
    if (typeof powerFailure === "string") {
      try {
        const parsed = JSON.parse(powerFailure);
                  ...
                  <div className="daily-section-header">
                    ...
                    <div className="daily-header-actions">
                      ...
                      <button
                        className="btn btn-small"
                        onClick={() =>
                          handleOpenRemarksModal(date, dailyLogs[0]?.remarks)
                        }
                              ...
                              <div key={idx} className="power-failure-item">
                                <div className="failure-info">
                                  <p>
                                    ...
                                    {pf.isFullDay && (
                                      <span className="full-day-badge">
                                        Full Day
                                      </span>
                                    )}
                                  </p>
                                  {pf.reason && (
                                    <p className="reason">{pf.reason}</p>
                                  )}
                                </div>
                              </div>
                            ))}
