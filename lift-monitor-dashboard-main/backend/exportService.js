...
export const generateExcelReport = (logs) => {
  ...
  Object.keys(logsByDate)
    .sort()
    .forEach((date) => {
      const dailyLogs = logsByDate[date].sort((a, b) =>
      ...
      const sheetName = new Date(date).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
    ...
    try {
      ...
      Object.keys(logsByDate)
        ...
        .forEach((date) => {
          ...
          // Get building name and create IST timestamp
          const building = dailyLogs[0]?.building || "Unknown Building";
          ...
          if (hasHT) {
            ...
            doc.text(
              ...
              { width: smallCell * 5 - 2, align: "center" }
            );
            ...
            ["R", "Y", "B", "PF", "Hz"].forEach((lbl, idx) => {
              doc
                .rect(
                  currentX + smallCell + smallCell * idx,
                  currentY + rowHeight * 2,
                  smallCell,
                  rowHeight
                )
              ...
              doc.text(
                ...
                { width: smallCell - 2, align: "center" }
              );
            });
            ...
            dailyLogs.forEach((log) => {
              if (log.htPanel) {
                ...
                doc.text(
                  log.time,
                  currentX + 1,
                  currentY + dataRowHeight / 2 - 2,
                  { width: timeCell - 2, align: "center" }
                );
                currentX += timeCell;
                ...
                const voltText = String(
                  log.htPanel.voltageFromWreb?.volt || "-"
                );
                doc.fontSize(voltText.length > 4 ? 5 : 6);
                doc.text(
                  voltText.substring(0, 6),
                  currentX + 1,
                  currentY + dataRowHeight / 2 - 2,
                  { width: smallCell - 2, align: "center" }
                );
                ...
                [
                  ...
                  doc
                    .rect(currentX, currentY, smallCell, dataRowHeight)
                    .stroke();
                  const valText = String(val || "-");
                  doc.fontSize(valText.length > 4 ? 4.5 : 6);
                  doc.text(
                    valText.substring(0, 5),
                    currentX + 1,
                    currentY + dataRowHeight / 2 - 2,
                    { width: smallCell - 2, align: "center" }
                  );
          ...
          if (hasLT) {
            ...
            dailyLogs.forEach((log) => {
              if (log.ltPanel) {
                ...
                [
                  log.ltPanel.incomer1,
                  log.ltPanel.incomer2,
                  log.ltPanel.incomer3,
                ].forEach((inc) => {
                  // Voltage - RY, YB, BR
...
// Generate PDFs grouped by building
export const generatePDFByBuilding = (logs) => {
  return new Promise((resolve, reject) => {
    try {
      if (!logs || logs.length === 0) {
        reject(new Error("No logs available for PDF generation"));
        return;
      }

      // Group logs by building
      const logsByBuilding = logs.reduce((acc, log) => {
        if (!acc[log.building]) {
          acc[log.building] = [];
        }
        acc[log.building].push(log);
        return acc;
      }, {});

      // Generate PDFs for each building
      const pdfPromises = Object.entries(logsByBuilding).map(
        ([building, buildingLogs]) =>
          generateSingleBuildingPDF(building, buildingLogs)
      );

      Promise.all(pdfPromises)
        .then((buffers) => {
          const result = {};
          Object.keys(logsByBuilding).forEach((building, index) => {
            result[building] = buffers[index];
          });
          resolve(result);
        })
        .catch(reject);
